...
class Model_pos extends CI_Model{
    ...
    function get_all_products(){
        ...
        $this->db->where('product_type_id !=',5);
    ...
    function fetch_holds_data(){
        ...
        $this->db->where("table_status",2);
        $this->db->where("tables.branch_auto_id",$this->_['branch_auto_id']);
        $this->db->where("tables.company_auto_id",$this->_['company_auto_id']);
        $data['tables'] = $this->db->get()->result_array();

        $this->db->select("hold_auto_id,hold_type,sales_auto_id,sales_id,sales_total,employee,customer,customer_auto_id,waiter_auto_id,waiter,employee ,check_in_time");
        $this->db->from("sales");
    ...
    function set_hold(){
        ...
        $session_data = array('sales_auto_id'=> $data['sales_auto_id'],'hold_auto_id'=> $data['hold_auto_id'],'hold_type'=>$data['hold_type']);
        $this->session->set_userdata($session_data);
        return $data;
    }
    ...
    function quick_pay(){
        $this->db->trans_begin();
        $sales_auto_id          = trim($this->input->post('sales_auto_id'));
        ...
        $sales_tender           = trim($this->input->post('sales_tender'));
        ...
        $sales_arr['pay_type']              = $pay_type;
        $sales_arr['sales_sub_total']       = $sub_total;
        $sales_arr['sales_tax']             = $tax;
        ...
        if ($cash != 0) {
            ...
            $arr['payment_auto_id']      = $sales_auto_id;
        ...
        if (!empty($payment_arr)) {
            $this->db->insert_batch('payments', $payment_arr); 
        }
    ...
    function fetch_all_customer(){
        ...
        $data = $this->db->query('SELECT customer_auto_id,customer_name,customer_secondary_code,customer_mobile FROM tbl_hrm_customers WHERE (customer_name LIKE "'.$search_string.'" OR customer_secondary_code LIKE "'.$search_string.'" OR customer_mobile LIKE "'.$search_string.'") AND company_auto_id = "'.$company_auto_id.'" and is_active = "2"')->result_array();
        if(!empty($data)){
            foreach($data as $val){
                $data_arr['suggestions'][] = array('value'=>$val["customer_secondary_code"].' | '.$val["customer_name"].' | '.$val["customer_mobile"],'customer_secondary_code'=>$val["customer_secondary_code"],'customer_name'=>$val["customer_name"],'customer_mobile'=>$val["customer_mobile"],'customer_auto_id'=>$val["customer_auto_id"]);
            }
        }
        return $data_arr;
    }
    ...
    function set_waiter(){
        ...
        if($this->db->trans_status()===0){
            $this->db->trans_rollback();
            $log = $this->logger->log_event(0,$this->lang->line('purchase_invoice_module'),5,$this->lang->line('common_delete_failed'),$this->lang->line('purchase_invoice_module'),0);
        }else{
            $this->db->trans_commit();
            $log = $this->logger->log_event(1,$this->lang->line('purchase_invoice_module'),2,$this->lang->line('common_delete_successfully'),$this->lang->line('purchase_invoice_module'),0);            
        }
        return $log;
    }
