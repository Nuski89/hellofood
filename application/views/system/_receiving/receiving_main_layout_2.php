...
<section class="content-header">
    <div class="row">
        <div class="col-md-10 operations-btn">
            <div class="box">
                <div class="row">
                    <div class="col-md-6 operation-filter-two">
                        <div class="input-group">
                            <input id="customer_auto_id" type="text" class="form-control" placeholder="[Alt+2] <?php echo $this->lang->line('supplier'); ?>">
                        ...
                        <table class="<?php echo pos_datatable_class(); ?> product_list_table_data" style="margin-top: 2px;">
                            <thead class="selected_table_row">
                                <tr>
                                    ...
                                    <th style="width: 100px;"><center><?php echo $this->lang->line('selling_price'); ?></center></th>
                                    <th style="width:170px;"><center><?php echo $this->lang->line('expiry_date');?></center></th>
            ...
            <div class="pos-tab-container">
                <div class="col-md-12 pos-tab-menu">
                    <div class="operation-btns">
                        <a class="list-group-item waves-effect waves-block text-center" onclick="set_hold('0~2')">
                            <i class="fa fa-shopping-bag fa-2x color"></i><br/> <?php echo $this->lang->line('new'); ?> <?php echo $this->lang->line('receiving'); ?>
                        </a>
            ...
            <div class="modal-body">
                <div class="row calculator">
                    ...
                    <div class="col-md-7 calculator-sums">
                        ...
                        <table id="cheque_number">
                            <tr>
                                <th width="20%"><label class="title"><?php echo$this->lang->line('bank');?></label></th>
                                <td width="30%"><input type="text" id="payment_cheque_bank" name="payment_cheque_bank" onfocus="this.select();"></td>
                                <th width="20%"><label class="title"><?php echo$this->lang->line('cheque_number');?></label></th>
                                <td width="30%"><input type="text" id="payment_cheque_no" name="payment_cheque_no" onfocus="this.select();"></td>
                            </tr>
                        </table>

                        <table id="credit_card">
                            <tr>
                                <th width="20%"><label class="title"><?php echo$this->lang->line('card_type');?></label></th>
                                <td width="30%">
                                    <select class="form-control" id="payment_card_type" name="payment_card_type">
                                ...
                                <th width="20%"><label class="title"><?php echo$this->lang->line('card_last_digits');?></label></th>
                                <td width="30%"><input type="text" id="payment_card_no" class="number" name="payment_card_no" onfocus="this.select();" maxlength="4"></td>
                            </tr>
                        </table>

                        <table id="gift_card">
                            <tr>
                                <th width="20%"><label class="title"><?php echo$this->lang->line('gift_code');?></label></th>
                                <td width="30%"><input type="text" id="payment_giftcard_code" name="payment_giftcard_code" onfocus="this.select();"></td>
                                <th width="20%"><label class="title"><?php echo$this->lang->line('gift_expire');?></label></th>
                                <td width="30%"><input type="text" id="payment_giftcard_expire" name="payment_giftcard_expire" onfocus="this.select();"></td>
                            </tr>
                        </table>
        ...
        <div class="modal-content">
            <div class="nav-tabs-custom">
                <ul class="nav nav-tabs pull-right">
                    <li><a data-toggle="tab" href="#tab_4"><?php echo $this->lang->line('product_group_arr')[5]; ?></a></li>
...
$(document).ready(function() {
    ...
    window.receiving_auto_id        = <?php echo json_encode($this->session->userdata('receiving_auto_id')); ?>;
    ...
    $('#register_form').bootstrapValidator({
        ...
        $.ajax({
            ...
            dataType: 'json',
            data: data,
            url: "<?php echo site_url('receiving/set_register'); ?>",
            beforeSend: function () {
                start_loading();
            },
    ...
    $('#register_close_form').bootstrapValidator({
        ...
        $.ajax({
            ...
            dataType: 'json',
            data: data,
            url: "<?php echo site_url('receiving/close_register'); ?>",
            beforeSend: function () {
                start_loading();
            },
            success: function (data) {
...
function add_payment(type){
    ...
    $('#bill-tender').val('');
    window.receiving_tender += tender;
    calculate_balance();
}
...
function edit_payment(type){
    if (type==2) {
        ...
        window.receiving_tender -= window.chaque;
        window.chaque          = 0;
        $('.chaque_total').html(to_currency(window.chaque));
    }else if(type==3){
        window.receiving_tender -= window.card;
        $('#bill-tender').val(window.card);
        window.card            = 0;
        $('.card_total').html(to_currency(window.card));
    }else if(type==4){
        window.receiving_tender -= window.gift_card;
        $('#bill-tender').val(window.gift_card);
        window.gift_card       = 0;
        $('.gift_card_total').html(to_currency(window.gift_card));
    }else{
        window.receiving_tender -= window.cash;
...
function fetch_holds_data(){
    $.ajax({
        type: 'GET',
        dataType: 'json',
        url: "<?php echo site_url('receiving/fetch_holds_data'); ?>",
        beforeSend: function () {
            ...
            if (!jQuery.isEmptyObject(data['holds'])) {
               ...
               var x =1;
                $.each(data['holds'], function (val, text) {
                    $('#bill_body').append('<tr><td>'+x+' </td><td>'+window.hold_types[text['hold_type']]+' '+(text['hold_type']==1 ?'Table ' :'')+text['hold_auto_id']+'</td><td>'+text['receiving_auto_id']+'</td><td>'+text['employee']+'</td><td>'+text['check_in_time']+'</td><td><a onclick="set_hold(\''+text['hold_auto_id']+'~'+text['hold_type']+'\')" class="btn btn-primary pull-right"><i class="fa fa-refresh" aria-hidden="true"></i></a></td></tr>');
                    if (window.hold_auto_id==text['hold_auto_id'] && window.hold_type==text['hold_type']) {
                        status = 0;
                        window.receiving_auto_id = text['receiving_auto_id'];
...
function close_register_modal(){
    $.ajax({
        type: 'GET',
        dataType: 'json',
        url: "<?php echo site_url('receiving/fetch_register_total'); ?>",
        beforeSend: function () {
            start_loading();
        },
        success: function (data) {
...
function clear_customer(){
    ...
    set_supplier(window.customer_auto_id,window.customer);
}
...
function set_hold(id){
    ...
    if (id) {
        ...
        $.ajax({
            ...
            url: "<?php echo site_url('receiving/set_hold'); ?>",
            beforeSend: function () {
                start_loading();
            },
            success: function (data) {
                stop_loading();
                window.receiving_auto_id = data['receiving_auto_id'];
...
function set_product_row(transaction_auto_id,is_status){
    if (transaction_auto_id) {
        if (is_status) {
            ...
            var selling_price       = $('.product_list_table_body #selling_price_'+transaction_auto_id).val();
            var expiry_date         = $('.product_list_table_body #expiry_date_'+transaction_auto_id).val();
        }else{
            ...
            var selling_price       = $('.modal_product_list_table_body #selling_price_'+transaction_auto_id).val();
            var expiry_date         = $('.modal_product_list_table_body #expiry_date_'+transaction_auto_id).val();
        }

        $.ajax({
            type: 'post',
            dataType: 'json',
            data:{'transaction_auto_id':transaction_auto_id,'qty':qty,'discount_percentage':discount_percentage,'amount':amount,'selling_price':selling_price,'expiry_date':expiry_date},
            url: "<?php echo site_url('receiving/set_product_row'); ?>",
            success: function (data) {
...
function add_to_cart(id,qty=1,discount_percentage=0){
    // window.receiving_auto_id!='0' ||
    if (window.hold_auto_id!='0' || window.hold_type!='0'){
        var discount_percentage = discount_percentage;
        var amount = window.product_arr[id]['product_cost'];
        $.ajax({
            type: 'post',
            dataType: 'json',
            data:{'id':id,'name':window.product_arr[id]['product_name'],'qty':qty,'discount_percentage':discount_percentage,'amount':amount,'category':window.product_arr[id]['product_category'],'cost':window.product_arr[id]['product_cost'],'transaction_type':2,'transaction_auto_id':window.transaction_auto_id,'type_id':window.product_arr[id]['product_type_id'],'number_of_persons':window.number_of_persons,'customer_auto_id':window.customer_auto_id,'customer':window.customer,'waiter_auto_id':window.waiter_auto_id,'waiter':window.waiter,'hold_type':window.hold_type,'hold_auto_id':window.hold_auto_id,'sales_auto_id':window.receiving_auto_id},
            url: "<?php echo site_url('receiving/add_to_cart'); ?>",
            beforeSend: function () {
                // start_loading();
            },
            success: function (data) {
    ...
    if (id) {
        swal({
            ...
            $.ajax({
                ...
                url: "<?php echo site_url('receiving/delete_transaction'); ?>",
                beforeSend: function () {
                    $('#product_overlay').show();
                },
                success: function (data) {
            ...
            if (dismiss === 'cancel') {
                $.ajax({
                    ...
                    dataType: 'json',
                    data :{'transaction_auto_id':id},
                    url: "<?php echo site_url('receiving/void_transaction'); ?>",
                    beforeSend: function () {
                        $('#product_overlay').show();
                    },
                    success: function (data) {
...
function fetch_register_data(){
    $.ajax({
        type: 'GET',
        dataType: 'json',
        url: "<?php echo site_url('receiving/fetch_register_data'); ?>",
        beforeSend: function () {
            start_loading();
        },
        success: function (data) {
...
function initialize_typeahead(){
    $('#waiter_auto_id').autocomplete({
        serviceUrl: '<?php echo site_url("receiving/fetch_all_waiters"); ?>',
    ...
    $('#customer_auto_id').autocomplete({
        serviceUrl: '<?php echo site_url("receiving/fetch_all_supplier"); ?>',
        minChars: 0,autoFocus: false,delay: 10,
        onSelect: function (data_arr) {
            ...
            set_supplier(data_arr.customer_auto_id,data_arr.customer_name);
        }
    });
...
function set_waiter(waiter_auto_id,waiter){
    $.ajax({
        type: 'GET',
        dataType: 'json',
        data:{'receiving_auto_id':window.receiving_auto_id,'waiter_auto_id':waiter_auto_id,'waiter':waiter},
        url: "<?php echo site_url('receiving/set_waiter'); ?>",
        beforeSend: function () {
            $('#product_overlay').show();
        },
        success: function (data) {
...
function set_supplier(customer_auto_id,customer){
    $.ajax({
        type: 'GET',
        dataType: 'json',
        data:{'receiving_auto_id':window.receiving_auto_id,'customer_auto_id':customer_auto_id,'customer':customer},
        url: "<?php echo site_url('receiving/set_supplier'); ?>",
        beforeSend: function () {
            $('#product_overlay').show();
        },
        success: function (data) {
...
function fetch_table_data(){
    ...
    window.item_discount    = 0;
    window.number_of_item   = 0;
    if (window.receiving_auto_id) {
        $.ajax({
            type: 'GET',
            dataType: 'json',
            data:{'receiving_auto_id':window.receiving_auto_id},
            url: "<?php echo site_url('receiving/fetch_table_data'); ?>",
            beforeSend: function () {
                $('#product_overlay').show();
            },
            success: function (data) {
                $('#pos_print_body').html(data.receiving_print);
                ...
                if (jQuery.isEmptyObject(data['data'])) {
                    ...
                    $('.product_list_table_body').append('<tr class="danger"><td colspan="8" class="text-center"><?php echo $this->lang->line('your_cart_empty'); ?> <div style="background-image: url(images/add-to-cart-empty-thumb.jpg);background-repeat: repeat; height:300px;"></div></td></tr>');
                }else {
                    $.each(data['data'], function (key, value) {
                        ...
                        if (value['status']==6) {
                            ...
                            $('#salse_transaction').append('<tr><td>'+(key+1)+'. # '+value['id']+' ( '+parseFloat(value['amount']).formatMoney(2, '.', ',')+' - '+value['discount']+' ) * '+value['qty']+'<span class="pull-right">'+parseFloat(value['net_amount']).formatMoney(2, '.', ',') + '</span><br>'+value['name']+'</td><td><center><a onclick="delete_transaction('+value['transaction_auto_id']+','+value['receiving_auto_id']+');"><i class="fa fa-trash-o fa-2x color"></i></a></center></td></tr>');
                            $('.product_list_table_body').append('<tr><td>'+(key+1)+'. '+value['name']+'</td><td><input type="text" class="form-control number chang amount_'+value['transaction_auto_id']+'" value="'+value['amount']+'" id="amount_'+value['transaction_auto_id']+'" onchange="set_product_row('+value['transaction_auto_id']+',1)" onfocus="this.select();"></td><td><input type="text" class="form-control number chang discount_'+value['transaction_auto_id']+'" value="'+value['discount_percentage']+'" id="discount_'+value['transaction_auto_id']+'" onchange="set_product_row('+value['transaction_auto_id']+',1)" onfocus="this.select();"></td><td><input type="text" class="form-control number chang qty_'+value['transaction_auto_id']+'" value="'+value['qty']+'" id="qty_'+value['transaction_auto_id']+'" onchange="set_product_row('+value['transaction_auto_id']+',1)" onfocus="this.select();"></td><td class="text-right net_amount_'+value['transaction_auto_id']+'"" id="net_amount_'+value['transaction_auto_id']+'">' + parseFloat(value['net_amount']).formatMoney(2, '.', ',') + '</td><td><input type="text" class="form-control number chang selling_price_'+value['transaction_auto_id']+'" value="'+value['selling_price']+'" id="selling_price_'+value['transaction_auto_id']+'" onchange="set_product_row('+value['transaction_auto_id']+',1)" onfocus="this.select();"></td><td><input type="text" style="font-size:20px;" class="form-control chang expiry_date_'+value['transaction_auto_id']+'" value="'+value['expiry_date']+'" id="expiry_date_'+value['transaction_auto_id']+'" onchange="set_product_row('+value['transaction_auto_id']+',1)" onfocus="this.select();" data-inputmask="\'alias\': \'yyyy-mm-dd\'" data-mask></td><td><center><button class="btn btn-default btn-sm" onclick="delete_transaction('+value['transaction_auto_id']+');"><i class="fa fa-trash-o fa-1x color"></i></button></center></td></tr>');
                            $('.modal_product_list_table_body').append('<tr><td>'+(key+1)+'. '+value['name']+'</td><td><input type="text" class="form-control number chang amount_'+value['transaction_auto_id']+'" value="'+value['amount']+'" id="amount_'+value['transaction_auto_id']+'" onchange="set_product_row('+value['transaction_auto_id']+',0)" onfocus="this.select();"></td><td><input type="text" class="form-control number chang discount_'+value['transaction_auto_id']+'" value="'+value['discount_percentage']+'" id="discount_'+value['transaction_auto_id']+'" onchange="set_product_row('+value['transaction_auto_id']+',0)" onfocus="this.select();"></td><td><input type="text" class="form-control number chang qty_'+value['transaction_auto_id']+'" value="'+value['qty']+'" id="qty_'+value['transaction_auto_id']+'" onchange="set_product_row('+value['transaction_auto_id']+',0)" onfocus="this.select();"></td><td class="text-right net_amount_'+value['transaction_auto_id']+'"" id="net_amount_'+value['transaction_auto_id']+'">' + parseFloat(value['net_amount']).formatMoney(2, '.', ',') + '</td><td><input type="text" class="form-control number chang selling_price_'+value['transaction_auto_id']+'" value="'+value['selling_price']+'" id="selling_price_'+value['transaction_auto_id']+'" onchange="set_product_row('+value['transaction_auto_id']+',1)" onfocus="this.select();"></td><td><input type="text" style="font-size:20px;" class="form-control chang expiry_date_'+value['transaction_auto_id']+'" value="'+value['expiry_date']+'" id="expiry_date_'+value['transaction_auto_id']+'" onchange="set_product_row('+value['transaction_auto_id']+',1)" onfocus="this.select();" data-inputmask="\'alias\': \'yyyy-mm-dd\'" data-mask></td><td><center><button class="btn btn-default btn-sm" onclick="delete_transaction('+value['transaction_auto_id']+');"><i class="fa fa-trash-o fa-1x color"></i></button></center></td></tr>');
...
function quick_pay(type,is_print) {
    ...
    if (window.total) {
        $.ajax({
            type: 'POST',
            dataType: 'json',
            data:{'pay_type':type,'receiving_auto_id':window.receiving_auto_id,'sub_total':window.sub_total,'item_discount':window.item_discount,'total':window.total,'full_discount':window.full_discount,'tax':window.tax,'cash':window.cash,'chaque':window.chaque,'card':window.card,'gift_card':window.gift_card,'receiving_tender':window.receiving_tender,'payment_cheque_bank':$('#payment_cheque_bank').val(),'payment_cheque_no':$('#payment_cheque_no').val(),'payment_card_type':$('#payment_card_type').val(),'payment_card_no':$('#payment_card_no').val(),'payment_giftcard_code':$('#payment_giftcard_code').val(),'payment_giftcard_expire':$('#payment_giftcard_expire').val()},
            url: "<?php echo site_url('receiving/quick_pay'); ?>",
            beforeSend: function () {
                $('#product_overlay').show();
            },
            success: function (data) {
                ...
                if (data['status']) {
                    if (is_print) {
                        $.ajax({
                            type: 'GET',
                            dataType: 'json',
                            data:{'sales_auto_id':window.receiving_auto_id},
                            url: "<?php echo site_url('receiving/fetch_table_data'); ?>",
                            beforeSend: function () {
                                $('#product_overlay').show();
                            },
...
function calculate_balance(){
    ...
    if (window.chaque>0) {
        ...
        $('#payment_cheque_bank').val('');
        $('#payment_cheque_no').val('');
        $('#cheque_number').hide();
    }
    if (window.card>0) {
        $('#credit_card').show();
    }else{
        $('#payment_card_type').val('');
        $('#payment_card_no').val('');
        $('#credit_card').hide();
    }
    if (window.gift_card>0) {
        $('#gift_card').show();
    }else{
        $('#payment_giftcard_code').val('');
        $('#payment_giftcard_expire').val('');
        $('#gift_card').hide();
    }
...
document.querySelector('#btn-cancel-void').onclick = function(){
    swal({
        ...
        $.ajax({
            ...
            dataType: 'json',
            data :{'receiving_auto_id':window.receiving_auto_id},
            url: "<?php echo site_url('receiving/delete_all_transaction'); ?>",
            beforeSend: function () {
                $('#product_overlay').show();
            },
            success: function (data) {
        ...
        if (dismiss === 'cancel') {
            $.ajax({
                ...
                dataType: 'json',
                data :{'receiving_auto_id':window.receiving_auto_id},
                url: "<?php echo site_url('receiving/void_all_transaction'); ?>",
                beforeSend: function () {
                    $('#product_overlay').show();
                },
                success: function (data) {
